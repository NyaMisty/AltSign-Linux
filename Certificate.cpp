...
{
	auto identifier = json[U("id")].as_string();
	auto attributes = json[U("attributes")];

	std::vector<unsigned char> data;
	if (attributes.has_field(U("certificateContent")))
	{
		auto encodedData = attributes[U("certificateContent")].as_string();
		data = base64_decode(encodedData);
	}

	auto machineName = attributes[U("machineName")].as_string();
	auto machineIdentifier = attributes[U("machineId")].as_string();
	...
	{
		auto name = attributes[U("name")].as_string();
		auto serialNumber = attributes[U("serialNumber")].as_string();

		_name = name.c_str();
		_serialNumber = serialNumber.c_str();
	}

	_identifier = std::make_optional(identifier.c_str());
	_machineName = std::make_optional(machineName.c_str());
	_machineIdentifier = std::make_optional(machineIdentifier.c_str());
}
