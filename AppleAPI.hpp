...
private:
	...
	T ProcessServicesResponse(web::json::value json, std::function<T(web::json::value)> parseHandler, std::function<std::optional<APIError>(int64_t)> resultCodeHandler)
	{
		...
		catch (std::exception& exception)
		{
			...
			if (json.has_field("resultCode"))
			{
				resultCode = json["resultCode"].as_integer();
			}
			else if (json.has_field("errorCode"))
			{
				resultCode = json["errorCode"].as_integer();
			}
			else
			{
				resultCode = -1;
			...
			if (json.has_field("userString"))
			{
				errorDescription = (json["userString"].as_string());
			}
			else if (json.has_field("resultString"))
			{
				errorDescription = (json["resultString"].as_string());
			}
			else if (json.has_field("errorMessage"))
			{
				errorDescription = (json["errorMessage"].as_string());
			}
			else if (json.has_field("errorId"))
			{
				errorDescription = (json["errorId"].as_string());
			}
			else
			{
			...
			throw LocalizedError((int)resultCode, ss.str() + " [" + exception.what() + "]");
		}
	}
