              ...
              auto certificates = this->ProcessServicesResponse<vector<shared_ptr<Certificate>>>(json, [](web::json::value json) -> vector<shared_ptr<Certificate>> 
				  {
					  ...
					  auto jsonArray = json["data"].as_array();
	...
	std::shared_ptr<Team> team)
{
	...
	std::map<utility::string_t, utility::string_t> headers = {
		...
		{"X-Apple-I-MD-RINFO", (std::to_string(session->anisetteData()->routingInfo())) },
		...
		{"X-Apple-Locale", (session->anisetteData()->locale()) },
				...
				.then([=](std::vector<unsigned char> compressedData)
					{
						...
						// odslog("[AppleAPI] retBody: " << decompressedXML);
...
pplx::task<json::value> AppleAPI::SendServicesRequest(std::string uri,
	...
	std::shared_ptr<Team> team)
{
	odslog("[AppleAPI] Querying ServiceAPI: " << uri);
	auto encodedParametersURI = web::uri::encode_uri("");
	uri_builder parametersBuilder(encodedParametersURI);
	parametersBuilder.append_query("teamId", (team->identifier()), true);

	for (auto pair : requestParameters)
	{
		parametersBuilder.append_query((pair.first), (pair.second), true);
	}

	auto query = parametersBuilder.query();

	auto json = web::json::value::object();
	json["urlEncodedQueryParams"] = web::json::value::string(query);
	...
	auto encodedURI = web::uri::encode_uri(uri);
	uri_builder builder(encodedURI);
				...
				.then([=](std::vector<unsigned char> decompressedData)
					{
						...
						// odslog("[AppleAPI] serviceAPI retBody: " << decompressedJSON);
